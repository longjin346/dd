...
## A term that is not listed here

Not an omission, and not a signal to work out what it might mean. The rule
in `references/extraction.md` already covers it: write the token exactly as
the thread wrote it. This file only ever narrows that default for terms
someone has confirmed.

## Adding an entry

An entry is added when a person who works with the term says what it
means. Not when a thread makes one reading look
